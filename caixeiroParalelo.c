...
void tsp (int depth, int current_length, int *path, int *min_distance) 
{
    int i;
        ...
        #pragma omp parallel
        {
            #pragma single
                ...
                for (i = 0; i < nb_towns; i++) 
                {
                    ...
                    {
                        int *newPath = (int*) malloc(sizeof(int) * nb_towns);
                        newPath[0] = 0;
                        newPath[depth] = town;
                        dist = d_matrix[me][i].dist;
                        #pragma omp task shared(min_distance) firstprivate(current_length,depth)
                            tsp (depth + 1, current_length + dist, newPath, min_distance);
                    }
                }
                end = clock();
                sequencialTime = ((double)(end - start))/CLOCKS_PER_SEC;
        }
    }
    else 
    {
        ...
        for (i = 0; i < nb_towns; i++) 
        {
            ...
            if (!present (town, depth, path)) 
            {
                path[depth] = town;
                dist = d_matrix[me][i].dist;
                tsp (depth + 1, current_length + dist, path, min_distance);
            }
        }
    }
}
